package com.example.fluorogallery;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Here we should indicate the condition
...
// Set the labels info manually
        ...
        cur_elt_array[0]="Pixel";
        cur_elt_array[1]="Brightness";
...
// se the data to be plotted and we should on our way
        ...
        //Here we should add our datapoints
        for (int y = 0; y < HSV.length; y++) {
            String strAmount=String.valueOf(100* HSV[y][2]);
            data_2_plot.add(strAmount) ;
        }
        //
        //data_2_plot.add("0.2") ;
        //data_2_plot.add("1.2") ;
...
// update the preview image in the layout
                    ...
                    int width_val = bitmap.getWidth();
                    int height_val = bitmap.getHeight();

                    //HEIGHT = width_val;

                    int[] R_matix = new int[width_val];
                    int[] G_matix = new int[width_val];
                    int[] B_matix = new int[width_val];

                    //int target_width_less = (int) width_val*0;
                    //int target_width_more = (int) width_val;//*3/5;

                    //int target_height_less = (int) height_val*0;
                    //int target_height_more = (int) height_val;//*4/5;
...
// calculate average of bitmap r,g,b values
                        R_matix[y] = (int) (redColors/pixelCount);
                        G_matix[y] = (int) (greenColors/pixelCount);
                        B_matix[y] = (int) (blueColors/pixelCount);
                    }
                    //String str = "";
                    //for (int i=target_height_less;i<target_height_more;i++){
                    //    str += (G_matix[i] + " ");
                    //}
                    ...
                    //
                    float [][] hsv = new float[width_val][3];
                    for (int y = 0; y < width_val; y++) {
                        Color.RGBToHSV(R_matix[y], G_matix[y], B_matix[y], hsv[y]);
                    }
                    HSV = hsv;
                    //Display maximum brightness
                    float max_val = 0;
                    for (int counter = 0; counter < HSV.length; counter++)
                    {
                        if (HSV[counter][2] > max_val)
                        {
                            max_val = HSV[counter][2];
                        }
                    }
                    String max_br = Float.toString(max_val);
                    textView.setText("Maximum brightness value: "+ max_br);
                    //Brightness
                    ...
                    //values.add(new Entry(i, hsv[i][2]));
                    //}
